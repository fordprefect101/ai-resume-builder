...
export class GdmLiveAudioVisuals extends LitElement {
  ...
  private visualize() {
    if (!this.canvas || !this.canvasCtx || !this.outputAnalyser || !this.inputAnalyser) {
  ...
  protected firstUpdated() {
    ...
    if (!canvasElement) {
      console.error('Visuals: Canvas element not found in shadow DOM.');
      return;
    }
    ...
    if (!canvasContext) {
      console.error('Visuals: 2D rendering context not available.');
      return;
    }
